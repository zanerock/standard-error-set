 ...
 * @param {Class} options.wrapUserErrorType - If set, then `URIError`, `RangeError`, and `TypeError` will be wrapped in 
 *   a new error of that `Class`. Otherwise, the logic will refer to the {@link commonErrorSettings} 
 *   `wrapUserErrorType`, which if undefined will result in the appropriate {@ArgumentInvalidError} analog.
 * @return {Array.<Error, boolean>} - An array containing either the original `Error` or the new wrapping `CommonError` 
